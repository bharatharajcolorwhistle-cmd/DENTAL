<?php
/**
 * Income Payment Status Management - Index Page
...
if (isset($_SESSION['income_payment_status_delete_success'])) {
    echo '<div class="alert alert-success alert-dismissible fade show" role="alert">';
    echo htmlspecialchars($_SESSION['income_payment_status_delete_success']);
    echo '<button type="button" class="btn-close" data-bs-dismiss="alert"></button>';
    echo '</div>';
    unset($_SESSION['income_payment_status_delete_success']);
}

if (isset($_SESSION['income_payment_status_delete_error'])) {
    echo '<div class="alert alert-danger alert-dismissible fade show" role="alert">';
    echo htmlspecialchars($_SESSION['income_payment_status_delete_error']);
    echo '<button type="button" class="btn-close" data-bs-dismiss="alert"></button>';
    echo '</div>';
    unset($_SESSION['income_payment_status_delete_error']);
}
?>
...
if (!empty($search)) {
    $where_conditions[] = "(ps.dcmt_name LIKE ? OR ps.dcmt_description LIKE ?)";
    $params[] = "%$search%";
    $params[] = "%$search%";
}
...
try {
    ...
    error_log("Income payment status fetch error: " . $e->getMessage());
    $payment_statuses = [];
    // Only show error message if it's a real database error, not a search issue
    if (empty($search)) {
        dcmt_show_message(trans('income_payment_status', 'load_error'), 'danger');
    }
}
?>
        ...
        <form method="GET" class="row g-3 align-items-end" id="searchForm">
            <div class="col-md-4">
                ...
                <input type="text" class="form-control dcmt-filter-field" id="search" name="search" 
                       ...
                       placeholder="<?php echo trans('income_payment_status', 'search_placeholder'); ?>"
        ...
        <?php if (!empty($search)): ?>
            <div class="mt-3">
                <div class="alert alert-info">
                    ...
                    <?php echo trans('income_payment_status', 'search_results_for'); ?>: <strong><?php echo htmlspecialchars($search); ?></strong>
                    <span class="badge bg-primary ms-2"><?php echo count($payment_statuses); ?> <?php echo trans('common', 'results'); ?></span>
                </div>
            </div>
        <?php endif; ?>
    </div>
...
<div class="card dcmt-records-table">
    <div class="card-header dcmt-view-card-header">
        <h6 class="dcmt-view-card-title">
            <?php echo trans('income_payment_status', 'income_payment_statuses'); ?>
        </h6>
        <a href="add.php" class="dcmt-add-form-view-all-link"><?php echo trans('income_payment_status', 'add_payment_status'); ?></a>
    </div>
    <div class="card-body">
        <?php if (empty($payment_statuses)): ?>
            <div class="text-center py-4">
                <h5 class="text-muted"><?php echo trans('income_payment_status', 'no_payment_statuses_found'); ?></h5>
                <p class="text-muted"><?php echo trans('income_payment_status', 'no_payment_statuses_message'); ?></p>
            </div>
        <?php else: ?>
                ...
                <table class="table table-hover">
                    <thead>
                        <tr>
                            ...
                            <?php endif; ?>
                            <th><?php echo trans('income_payment_status', 'name'); ?></th>
                            <th><?php echo trans('common', 'description'); ?></th>
                            <th><?php echo trans('common', 'status'); ?></th>
                            <th><?php echo trans('income_payment_status', 'usage_count'); ?></th>
                    ...
                    <tbody>
                        <?php foreach ($payment_statuses as $status): ?>
                            <?php
                            $is_system_item = (($status['dcmt_created_by'] ?? '') === 'system');
                            $is_used_item = ((int) $status['income_count'] > 0);
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <input type="checkbox" class="form-check-input dcmt-income-payment-status-checkbox"
                                           value="<?php echo (int) $status['dcmt_id']; ?>"
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div>
                                            <?php 
                                            // Check if this is a system default payment status
                                            $display_name = $status['dcmt_name'];
                                            if ($status['dcmt_created_by'] === 'system') {
                                                // Try to translate the name, fallback to original if translation not found
                                                $translated_name = trans('income_payment_status', $display_name);
                                ...
                                <td>
                                    <?php if (!empty($status['dcmt_description'])): ?>
                                        <?php echo htmlspecialchars($status['dcmt_description']); ?>
                                    <?php else: ?>
                                        <span class="text-muted">-</span>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <span class="text-<?php echo $status['dcmt_status'] === 'active' ? 'success' : 'secondary'; ?>">
                                        <?php echo trans('common', $status['dcmt_status']); ?>
                                    </span>
                                </td>
                                <td>
                                    <span class="text-info"><?php echo $status['income_count']; ?> <?php echo trans('income_payment_status', 'income_records'); ?></span>
                                </td>
                                <td>
                                    <?php echo dcmt_format_date($status['dcmt_created_at']); ?>
                                </td>
                                <td>
                                    <div class="btn-group btn-group-sm btn-group-action" role="group">
                                        <a href="edit.php?id=<?php echo $status['dcmt_id']; ?>" 
                                           class="btn" 
                                        ...
                                        <?php if ($is_used_item && !$is_system_item): ?>
                                            <button type="button" 
                                                    ...
                                                    title="<?php echo trans('income_payment_status', 'cannot_delete_in_use'); ?>"
                                                    disabled>
                                        ...
                                        <?php elseif ($can_delete_item): ?>
                                            <button type="button" 
                                                    ...
                                                    onclick="showDeleteModal(<?php echo $status['dcmt_id']; ?>, '<?php echo htmlspecialchars($status['dcmt_name'], ENT_QUOTES); ?>', 'income_payment_status')">
        ...
        <div class="modal-content">
            <div class="modal-header bg-danger text-white">
                <h5 class="modal-title" id="deleteModalLabel">
                    <i class="fas fa-exclamation-triangle me-2"></i><?php echo trans('income_payment_status', 'confirm_deletion'); ?>
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p><?php echo trans('income_payment_status', 'confirm_delete_warning'); ?></p>
                <div class="alert alert-warning">
                    <i class="fas fa-exclamation-triangle me-2"></i>
                    <strong><?php echo trans('common', 'warning'); ?>:</strong> <?php echo trans('income_payment_status', 'delete_warning_message'); ?>
                </div>
                <p><strong><?php echo trans('income_payment_status', 'payment_status'); ?>:</strong> <span id="deleteItemName"></span></p>
            </div>
...
window.translations = {
    ...
    confirm_delete_single: <?php echo json_encode(trans('income_payment_status', 'confirm_delete_single'), JSON_UNESCAPED_UNICODE); ?>,
    confirm_delete_multiple: <?php echo json_encode(trans('income_payment_status', 'confirm_delete_multiple'), JSON_UNESCAPED_UNICODE); ?>
};
...
function getBulkCheckboxes() {
    return document.querySelectorAll('.dcmt-income-payment-status-checkbox');
}

function updateBulkActions() {
    const checkboxes = getBulkCheckboxes();
    const checkedBoxes = document.querySelectorAll('.dcmt-income-payment-status-checkbox:checked');
...
function bulkDelete() {
    ...
    if (checkedBoxes.length === 0) {
        alert(<?php echo json_encode(trans('income_payment_status', 'please_select_one_record'), JSON_UNESCAPED_UNICODE); ?>);
        return;
    }
...
function bulkDeleteAjax(itemIds) {
    if (typeof showLoadingMessage === 'function') {
        showLoadingMessage(<?php echo json_encode(trans('income_payment_status', 'deleting_records'), JSON_UNESCAPED_UNICODE); ?>);
    }
    ...
    .then(data => {
        ...
        const errorMessage = data.message || <?php echo json_encode(trans('income_payment_status', 'failed_to_delete_records'), JSON_UNESCAPED_UNICODE); ?>;
    ...
    .catch(error => {
        ...
        const errorMessage = <?php echo json_encode(trans('income_payment_status', 'error_occurred_deleting_records'), JSON_UNESCAPED_UNICODE); ?>;
        if (typeof showErrorMessage === 'function') {
            showErrorMessage(errorMessage);
        } else {
            alert(errorMessage);
