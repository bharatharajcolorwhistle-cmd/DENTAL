<?php
/**
 * Edit Expense Payment Method Page
...
try {
    $sql = "SELECT * FROM dcmt_expense_payment_methods WHERE dcmt_id = ?";
    ...
    if (!dcmt_verify_csrf_token($_POST['csrf_token'])) {
        ...
        if ($validation_result['valid']) {
            ...
            if (!$is_system_default) {
                try {
                    $stmt = $dcmt_pdo->prepare("SELECT dcmt_id FROM dcmt_expense_payment_methods WHERE dcmt_name = ? AND dcmt_id != ?");
            ...
            if (empty($errors)) {
                try {
                    $sql = "UPDATE dcmt_expense_payment_methods SET dcmt_description = ?, dcmt_status = ?, dcmt_updated_at = NOW() WHERE dcmt_id = ?";
                    ...
                    // Log activity
                    dcmt_log_activity("Expense payment method updated: $name", "Expense Payment Method ID: $payment_method_id, Name: $name, Status: $status");
                    
                    // Set success message and redirect BEFORE any output
                    dcmt_show_message(trans('expense_payment_method', 'update_success'), "success");
                    header('Location: index.php');
                    exit();
                    
                } catch (PDOException $e) {
                    $errors[] = 'Database error: ' . $e->getMessage();
                    dcmt_log_activity("Error updating expense payment method: " . $e->getMessage(), "error");
                }
            }
        } else {
...
if ($is_system_default) {
    $translated_name = trans('expense_payment_method', $display_name);
...
<div class="dcmt-add-form-container">
    <div class="dcmt-add-form-header">
        <div class="dcmt-add-form-header-content">
            <h1 class="dcmt-add-form-page-title"><?php echo trans('expense_payment_method', 'edit_payment_method'); ?></h1>
        ...
        <form method="POST" action="" id="paymentMethodForm">
            ...
            <div class="row">
                <div class="col-md-8">
                    <div class="mb-3">
                        <label for="name" class="form-label"><?php echo trans('expense_payment_method', 'name'); ?> <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="name" name="name" 
                               ...
                               maxlength="100" placeholder="<?php echo trans('expense_payment_method', 'enter_name'); ?>"
                               style="<?php echo $is_system_default ? 'background-color: #f8f9fa; cursor: not-allowed;' : ''; ?>">
                        <?php if ($is_system_default): ?>
                            <div class="form-text text-warning">
                                <i class="fas fa-lock me-1"></i><?php echo trans('expense_payment_method', 'name_locked'); ?>
                            </div>
                        <?php else: ?>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        <label for="status" class="form-label"><?php echo trans('expense_payment_method', 'status'); ?> <span class="text-danger">*</span></label>
                ...
                <div class="col-md-12">
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="description" name="description" 
                                  rows="3" maxlength="500" placeholder="<?php echo trans('expense_payment_method', 'enter_description'); ?>"><?php echo htmlspecialchars($form_data['description']); ?></textarea>
            ...
            <div class="dcmt-form-actions">
                ...
                <button type="submit" class="btn dcmt-btn-submit" id="submitBtn">
                    <i class="fas fa-save"></i><?php echo trans('expense_payment_method', 'update_payment_method_record'); ?>
                </button>
            </div>
        </form>
</div>
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            if (submitBtn) {
                ...
                submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i><?php echo trans('expense_payment_method', 'updating'); ?>...';
            }
        });
    }
});
