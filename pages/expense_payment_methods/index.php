...
if (isset($_SESSION['expense_payment_method_delete_success'])) {
    echo '<div class="alert alert-success alert-dismissible fade show" role="alert">';
    echo htmlspecialchars($_SESSION['expense_payment_method_delete_success']);
    echo '<button type="button" class="btn-close" data-bs-dismiss="alert"></button>';
    echo '</div>';
    unset($_SESSION['expense_payment_method_delete_success']);
}

if (isset($_SESSION['expense_payment_method_delete_error'])) {
    echo '<div class="alert alert-danger alert-dismissible fade show" role="alert">';
    echo htmlspecialchars($_SESSION['expense_payment_method_delete_error']);
    echo '<button type="button" class="btn-close" data-bs-dismiss="alert"></button>';
    echo '</div>';
    unset($_SESSION['expense_payment_method_delete_error']);
}
?>
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        <?php foreach ($payment_methods as $method): ?>
                            ...
                            $is_used_item = ((int) $method['expense_count'] > 0);
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <input type="checkbox" class="form-check-input dcmt-expense-payment-method-checkbox"
...
window.translations = {
    ...
    confirm_delete_single: <?php echo json_encode(trans('expense_payment_method', 'confirm_delete_single'), JSON_UNESCAPED_UNICODE); ?>,
    confirm_delete_multiple: <?php echo json_encode(trans('expense_payment_method', 'confirm_delete_multiple'), JSON_UNESCAPED_UNICODE); ?>
};
...
function getBulkCheckboxes() {
    return document.querySelectorAll('.dcmt-expense-payment-method-checkbox');
}

function updateBulkActions() {
    const checkboxes = getBulkCheckboxes();
    const checkedBoxes = document.querySelectorAll('.dcmt-expense-payment-method-checkbox:checked');
...
function bulkDelete() {
    ...
    if (checkedBoxes.length === 0) {
        alert(<?php echo json_encode(trans('expense_payment_method', 'please_select_one_record'), JSON_UNESCAPED_UNICODE); ?>);
        return;
    }
...
function bulkDeleteAjax(itemIds) {
    if (typeof showLoadingMessage === 'function') {
        showLoadingMessage(<?php echo json_encode(trans('expense_payment_method', 'deleting_records'), JSON_UNESCAPED_UNICODE); ?>);
    }
    ...
    .then(data => {
        ...
        const errorMessage = data.message || <?php echo json_encode(trans('expense_payment_method', 'failed_to_delete_records'), JSON_UNESCAPED_UNICODE); ?>;
    ...
    .catch(error => {
        ...
        const errorMessage = <?php echo json_encode(trans('expense_payment_method', 'error_occurred_deleting_records'), JSON_UNESCAPED_UNICODE); ?>;
        if (typeof showErrorMessage === 'function') {
            showErrorMessage(errorMessage);
        } else {
            alert(errorMessage);
