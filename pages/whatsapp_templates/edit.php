...
if ($template_id > 0) {
    $stmt = $dcmt_pdo->prepare('SELECT * FROM dcmt_whatsapp_templates WHERE dcmt_id = ?');
    $stmt->execute([$template_id]);
    $template = $stmt->fetch(PDO::FETCH_ASSOC);
}
if (!$template) {
    dcmt_show_message(trans('whatsapp_template', 'template_not_found'), 'danger');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!dcmt_verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            try {
                $check = $dcmt_pdo->prepare('SELECT dcmt_id FROM dcmt_whatsapp_templates WHERE dcmt_name = ? AND dcmt_id != ?');
                $check->execute([$name, $template_id]);
                if ($check->fetch()) {
                    $errors[] = trans('whatsapp_template', 'name_exists');
                } else {
                    $stmt = $dcmt_pdo->prepare('
                        UPDATE dcmt_whatsapp_templates
                        SET dcmt_name = ?, dcmt_message = ?, dcmt_status = ?, dcmt_updated_at = NOW()
                        WHERE dcmt_id = ?
                    ');
                    $stmt->execute([$name, $message, $status, $template_id]);
                    dcmt_show_message(trans('whatsapp_template', 'update_success'), 'success');
                    dcmt_redirect('index.php');
                }
        ...
        $template['dcmt_name'] = $name;
        $template['dcmt_message'] = $message;
        $template['dcmt_status'] = $status;
    }
}
...
<div class="dcmt-add-form-container">
    <div class="dcmt-add-form-header">
        <div class="dcmt-add-form-header-content">
            <h1 class="dcmt-add-form-page-title"><?php echo trans('whatsapp_template', 'edit_template'); ?></h1>
    ...
    <form method="POST">
            ...
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="name" name="name" maxlength="100" required
                           value="<?php echo htmlspecialchars($template['dcmt_name'] ?? ''); ?>">
                </div>
            </div>
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <select class="form-select" id="status" name="status" required>
                        <option value="active" <?php echo ($template['dcmt_status'] ?? '') === 'active' ? 'selected' : ''; ?>><?php echo trans('common', 'active'); ?></option>
                        <option value="inactive" <?php echo ($template['dcmt_status'] ?? '') === 'inactive' ? 'selected' : ''; ?>><?php echo trans('common', 'inactive'); ?></option>
                    </select>
                </div>
            </div>
        </div>
        <div class="mb-3">
            ...
            <textarea class="form-control" id="message" name="message" rows="5" maxlength="4000" required><?php echo htmlspecialchars($template['dcmt_message'] ?? ''); ?></textarea>
        ...
        <div class="dcmt-form-actions">
            ...
            <button type="submit" class="btn dcmt-btn-submit"><i class="fas fa-save"></i><?php echo trans('whatsapp_template', 'update_template_record'); ?></button>
        </div>
    </form>
</div>
