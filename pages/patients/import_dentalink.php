<?php
/**
 * Import Patients — Dentalink CSV export format
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['import_patients_dentalink'])) {
    // Verify CSRF token
    if (!dcmt_verify_csrf_token($_POST['csrf_token'])) {
        ...
        if (!isset($_FILES['csv_file']) || $_FILES['csv_file']['error'] !== UPLOAD_ERR_OK) {
            ...
            if ($file_extension !== 'csv') {
                ...
                // Process CSV file
                $result = processPatientImportDentalink($file['tmp_name']);
...
/**
 * Process a Dentalink-export CSV (Spanish headers; optional preamble rows).
 */
function processPatientImportDentalink($file_path)
{
    global $dcmt_pdo;
    ...
    $normalizeDentalinkDateOfBirth = static function ($raw) {
        $raw = trim((string) $raw);
        if ($raw === '') {
            return '';
        }
        ...
        // Dentalink uses US-style month/day/year (e.g. 7/3/2018 → July 3, 2018).
    ...
    $header_map = [
        ...
        'apellidos' => 'fathers_last_name',
        'gender' => 'gender',
        'sexo' => 'gender',
        'date_of_birth' => 'date_of_birth',
        'dob' => 'date_of_birth',
        'fecha de nac.' => 'date_of_birth',
        'fecha de nac' => 'date_of_birth',
        'fecha de nacimiento' => 'date_of_birth',
        'fecha nacimiento' => 'date_of_birth',
        'fecha nac.' => 'date_of_birth',
        'fecha nac' => 'date_of_birth',
        'fecha de nacimiento / dob' => 'date_of_birth',
        'age' => 'age',
        'height_cm' => 'height_cm',
        'weight_kg' => 'weight_kg',
        'email' => 'email',
        ...
        '# apoderado' => 'emergency_contact_name',
    ...
    $cols = [];
    $headers = null;
    for ($hdr_attempt = 0; $hdr_attempt < 20; $hdr_attempt++) {
        ...
        $normalized_headers = array_map($normalizeCsvHeaderCell, $candidate);
        $probe_cols = [];
        ...
        if (isset($probe_cols['full_name']) || isset($probe_cols['first_name'])) {
            foreach ($normalized_headers as $index => $header) {
                if (!isset($probe_cols['date_of_birth']) && preg_match('/fecha\s*(de\s*)?nac/i', $header)) {
                    $probe_cols['date_of_birth'] = $index;
                    break;
                }
            }
            $headers = $candidate;
    ...
    if (!$headers || (!isset($cols['full_name']) && !isset($cols['first_name']))) {
        $errors[] = trans('patient', 'missing_dentalink_name_column');
        fclose($handle);
        return ['errors' => $errors, 'success' => $success, 'imported_count' => 0, 'updated_income_count' => 0];
    }
    ...
    while (($data = fgetcsv($handle, 0, ',', '"', '\\')) !== FALSE) {
        ...
        $full_name = isset($cols['full_name']) ? trim($data[$cols['full_name']] ?? '') : '';
        ...
        $phone_raw = isset($cols['phone']) ? trim($data[$cols['phone']] ?? '') : '';
        $phone = $normalizePhoneToMexicoE164($phone_raw);
        $address = isset($cols['address']) ? trim($data[$cols['address']] ?? '') : '';
        if ($address === '') {
            $addr_parts = [];
            if (isset($cols['addr_city'])) {
                $addr_parts[] = trim($data[$cols['addr_city']] ?? '');
            }
            if (isset($cols['addr_comuna'])) {
                $addr_parts[] = trim($data[$cols['addr_comuna']] ?? '');
            }
            if (isset($cols['addr_line'])) {
                $addr_parts[] = trim($data[$cols['addr_line']] ?? '');
            }
            $addr_parts = array_values(array_filter($addr_parts, static function ($p) {
                return $p !== '';
            }));
            if (!empty($addr_parts)) {
    ...
    if ($imported_count > 0 || $updated_income_count > 0) {
        dcmt_log_activity("Dentalink patients imported: $imported_count new, $updated_income_count income records linked", "patients_imported_dentalink");
    }
        ...
        <div class="col-12 mb-2">
            <div class="dcmt-page-header">
                <h4 class="dcmt-page-title">
                    <i class="fas fa-upload me-2"></i><?php echo trans('patient', 'import_patients_dentalink'); ?>
                </h4>
            </div>
        </div>
    </div>
                ...
                <div class="card-body">
                    ...
                    <form action="" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                        ...
                        <div class="d-flex gap-2">
                            <button type="submit" name="import_patients_dentalink" class="btn btn-primary">
        ...
        <div class="col-lg-4">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <p class="small"><?php echo trans('patient', 'dentalink_required_help'); ?></p>
                    ...
                    <div class="mt-3 d-flex flex-column gap-2">
                        <a href="download_dentalink_template.php" class="btn btn-outline-primary btn-sm">
                            <i class="fas fa-download me-2"></i><?php echo trans('patient', 'download_dentalink_template'); ?>
                        </a>
                        <a href="import.php" class="btn btn-outline-secondary btn-sm">
                            <?php echo trans('patient', 'standard_csv_import'); ?>
                        </a>
                    </div>
                </div>
            </div>
