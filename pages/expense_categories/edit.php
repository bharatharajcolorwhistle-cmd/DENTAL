...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$is_system_default && empty($name)) {
        $errors[] = trans('expense_category', 'name_required');
    }
    
    if (!$is_system_default && strlen($name) > 100) {
        $errors[] = trans('expense_category', 'name_too_long');
    }
    ...
    if (empty($errors) && !$is_system_default) {
        try {
            $stmt = $dcmt_pdo->prepare("SELECT dcmt_id FROM dcmt_expense_categories WHERE dcmt_name = ? AND dcmt_id != ?");
            $stmt->execute([$name, $category_id]);
            if ($stmt->fetch()) {
                $errors[] = trans('expense_category', 'name_already_exists');
            }
        } catch (PDOException $e) {
            error_log("Error checking expense category name: " . $e->getMessage());
            $errors[] = trans('expense_category', 'error_checking_name');
        }
    }
        ...
        try {
            ...
            if ($column_exists) {
                ...
                $stmt = $dcmt_pdo->prepare("
                    UPDATE dcmt_expense_categories 
                    SET dcmt_name = ?, dcmt_description = ?, dcmt_status = ?, dcmt_parent_category_id = ?, dcmt_updated_at = NOW()
                    WHERE dcmt_id = ?
                ");
                $stmt->execute([$name, $description, $status, $parent_category_id, $category_id]);
            } else {
                // Old format without parent_category_id
                $stmt = $dcmt_pdo->prepare("
                    UPDATE dcmt_expense_categories 
                    SET dcmt_name = ?, dcmt_description = ?, dcmt_status = ?, dcmt_updated_at = NOW()
                    WHERE dcmt_id = ?
                ");
                $stmt->execute([$name, $description, $status, $category_id]);
            }
            
            dcmt_log_activity(trans('expense_category', 'category_updated'), "Category ID: $category_id, Name: $name");
            dcmt_show_message(trans('expense_category', 'category_updated_successfully'), 'success');
            dcmt_redirect('index.php');
            
        } catch (PDOException $e) {
            error_log("Error updating expense category: " . $e->getMessage());
            $errors[] = trans('expense_category', 'error_updating_category');
        }
    }
}
...
<head>
    ...
    <title><?php echo trans('expense_category', 'edit_category'); ?> - <?php echo dcmt_get_site_name(); ?></title>
        ...
        <div class="dcmt-add-form-container">
            <div class="dcmt-add-form-header">
                <div class="dcmt-add-form-header-content">
                    <h1 class="dcmt-add-form-page-title"><?php echo trans('expense_category', 'edit_category'); ?></h1>
                        ...
                        <form method="POST" action="">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="name" class="form-label">
                                            <i class="fas fa-tag me-2"></i><?php echo trans('expense_category', 'category_name'); ?> <span class="text-danger">*</span>
                                        </label>
                                        <?php
                                        // Get translated name for display
                                        $display_name = $category['dcmt_name'] ?? '';
                                        if ($is_system_default) {
                                            $translated_name = trans('expense_category', $display_name);
                                        ...
                                        <?php if ($is_system_default): ?>
                                            <div class="form-text text-warning">
                                                <i class="fas fa-lock me-1"></i><?php echo trans('expense_category', 'name_locked'); ?>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="status" class="form-label">
                                            <i class="fas fa-toggle-on me-2"></i><?php echo trans('expense_category', 'status'); ?>
                                        </label>
                            ...
                            <div class="mb-3">
                                <label for="description" class="form-label">
                                    <i class="fas fa-align-left me-2"></i><?php echo trans('common', 'description'); ?>
                                </label>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="parent_category_id" name="parent_category_id">
                                    ...
                                    <?php
                                    // Fetch main categories (categories without parent, excluding current category)
                                    try {
                                        ...
                                        foreach ($parent_categories as $parent) {
                                            $selected = (isset($category['dcmt_parent_category_id']) && $category['dcmt_parent_category_id'] == $parent['dcmt_id']) ? 'selected' : '';
                            ...
                            <div class="dcmt-form-actions">
                                ...
                                <button type="submit" class="btn dcmt-btn-submit" id="submitBtn">
                                    <i class="fas fa-plus"></i><?php echo trans('expense_category', 'update_category_record'); ?>
                                </button>
                            </div>
                        </form>
        </div>
    ...
    function resetForm() {
        ...
        setTimeout(() => {
            ...
            const fields = {
                ...
                'parent_category_id': '<?php echo $category['dcmt_parent_category_id'] ?? ''; ?>'
            };
