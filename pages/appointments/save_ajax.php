...
if (!in_array($status, $allowed_status, true)) {
    echo json_encode(['success' => false, 'field' => 'status', 'message' => $m['status_invalid']]);
    exit();
}
...
try {
    ...
    if (dcmt_has_appointment_overlap($dcmt_pdo, $doctor_id, $start_at, $end_at, $exclude_id)) {
        echo json_encode(['success' => false, 'fields' => ['start_time', 'end_time'], 'message' => $m['slot_booked']]);
        exit();
    }
    ...
    if ($action === 'update') {
        ...
        $stmt = $dcmt_pdo->prepare("
            ...
            SET dcmt_patient_id = ?, dcmt_doctor_id = ?, dcmt_start_at = ?, dcmt_end_at = ?,
                dcmt_status = ?, dcmt_reason = ?, dcmt_notes = ?
            WHERE dcmt_id = ?
        ");
        $stmt->execute([$patient_id, $doctor_id, $start_at, $end_at, $status, $reason !== '' ? $reason : null, $notes !== '' ? $notes : null, $appointment_id]);
    ...
    $stmt = $dcmt_pdo->prepare("
        ...
        (dcmt_patient_id, dcmt_doctor_id, dcmt_start_at, dcmt_end_at, dcmt_status, dcmt_reason, dcmt_notes, dcmt_created_by)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ");
    $stmt->execute([
        $patient_id,
