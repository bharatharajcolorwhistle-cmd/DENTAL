<?php
/**
 * AJAX Delete Income Payment Method
...
if ($payment_method_id <= 0) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => trans('income_payment_method', 'invalid_payment_method_id')]);
    exit();
}
...
try {
    ...
    $stmt = $dcmt_pdo->prepare("
        SELECT pm.*, COUNT(i.dcmt_id) as usage_count 
    ...
    if (!$payment_method) {
        ...
        echo json_encode(['success' => false, 'message' => trans('income_payment_method', 'payment_method_not_found')]);
        exit();
    }
    ...
    if ($payment_method['usage_count'] > 0) {
        ...
        echo json_encode([
            'success' => false, 
            'message' => trans('income_payment_method', 'cannot_delete_in_use'),
    ...
    $delete_stmt = $dcmt_pdo->prepare("DELETE FROM dcmt_income_payment_methods WHERE dcmt_id = ?");
    $delete_result = $delete_stmt->execute([$payment_method_id]);
    
    if ($delete_result && $delete_stmt->rowCount() > 0) {
        // Log activity
        dcmt_log_activity(
            "Income payment method deleted: " . $payment_method['dcmt_name'], 
            "Income Payment Method ID: $payment_method_id, Name: " . $payment_method['dcmt_name']
        );
        ...
        echo json_encode(['success' => false, 'message' => trans('income_payment_method', 'delete_error')]);
    }
    
} catch (PDOException $e) {
    error_log("Error deleting income payment method: " . $e->getMessage());
    http_response_code(500);
