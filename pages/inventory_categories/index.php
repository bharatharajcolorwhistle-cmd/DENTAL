<?php
/**
 * Inventory Categories Management - Index Page
...
if (isset($_SESSION['inventory_category_delete_success'])) {
    echo '<div class="alert alert-success alert-dismissible fade show" role="alert">';
    echo htmlspecialchars($_SESSION['inventory_category_delete_success']);
    echo '<button type="button" class="btn-close" data-bs-dismiss="alert"></button>';
    echo '</div>';
    unset($_SESSION['inventory_category_delete_success']);
}

if (isset($_SESSION['inventory_category_delete_error'])) {
    echo '<div class="alert alert-danger alert-dismissible fade show" role="alert">';
    echo htmlspecialchars($_SESSION['inventory_category_delete_error']);
    echo '<button type="button" class="btn-close" data-bs-dismiss="alert"></button>';
    echo '</div>';
    unset($_SESSION['inventory_category_delete_error']);
}
?>
...
try {
    // Get inventory categories with usage counts
    $sql = "
        SELECT c.*, 
               COUNT(DISTINCT i.dcmt_id) as inventory_count
        FROM dcmt_inventory_categories c
        LEFT JOIN dcmt_inventory i ON c.dcmt_id = i.dcmt_category_id
        $where_clause
        GROUP BY c.dcmt_id
    ...
    error_log("Inventory categories fetch error: " . $e->getMessage());
    $categories = [];
    // Only show error message if it's a real database error, not a search issue
    if (empty($search)) {
        dcmt_show_message(trans('inventory_category', 'load_error'), 'danger');
    }
}
?>
        ...
        <form method="GET" class="row g-3 align-items-end" id="searchForm">
            <div class="col-md-4">
                ...
                <input type="text" class="form-control dcmt-filter-field" id="search" name="search" 
                       ...
                       placeholder="<?php echo trans('inventory_category', 'search_placeholder'); ?>"
        ...
        <?php if (!empty($search)): ?>
            <div class="mt-3">
                <div class="alert alert-info">
                    ...
                    <?php echo trans('inventory_category', 'search_results_for'); ?>: <strong><?php echo htmlspecialchars($search); ?></strong>
...
<div class="card dcmt-records-table">
    <div class="card-header dcmt-view-card-header">
        <h6 class="dcmt-view-card-title">
            <i class="fas fa-boxes dcmt-view-card-title-icon"></i><?php echo trans('inventory_category', 'inventory_categories'); ?>
        </h6>
        <a href="add.php" class="dcmt-add-form-view-all-link"><?php echo trans('inventory_category', 'add_category'); ?></a>
    </div>
    <div class="card-body">
        <?php if (empty($categories)): ?>
            <div class="text-center py-4">
                <i class="fas fa-tags fa-3x text-muted mb-3"></i>
                <h5 class="text-muted"><?php echo trans('inventory_category', 'no_categories_found'); ?></h5>
                <p class="text-muted"><?php echo trans('inventory_category', 'start_adding_category'); ?></p>
            </div>
        <?php else: ?>
                        ...
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <?php endif; ?>
                                    <th><?php echo trans('inventory_category', 'category_name'); ?></th>
                                    ...
                                    <th><?php echo trans('inventory_category', 'usage'); ?></th>
                            ...
                            <?php foreach ($categories as $category): ?>
                                ...
                                $is_used_category = ((int) $category['inventory_count'] > 0);
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <input type="checkbox" class="form-check-input dcmt-inventory-category-checkbox"
                                    ...
                                    <td>
                                        ...
                                        if (isset($category['dcmt_created_by']) && $category['dcmt_created_by'] === 'system') {
                                            $translated_name = trans('inventory_category', $category_name);
                                    ...
                                    <td>
                                        <span class="text-<?php echo $category['dcmt_product_type'] === 'for_sale' ? 'success' : 'info'; ?>">
                                            <?php echo trans('inventory_category', $category['dcmt_product_type']); ?>
                                        </span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <span class="text-info"><?php echo $category['inventory_count']; ?> <?php echo trans('inventory_category', 'items'); ?></span>
                                    </td>
                                    <td>
                                        ...
                                        <div class="btn-group btn-group-sm btn-group-action" role="group">
                                            ...
                                            <?php if ($is_used_category && !$is_system_category): ?>
                                                <button type="button" 
                                                        ...
                                                        title="<?php echo trans('inventory_category', 'cannot_delete_used_category'); ?>"
                                                        disabled>
                                            ...
                                            <?php elseif ($can_delete_item): ?>
                                                <button type="button" 
                                                        ...
                                                        onclick="showDeleteModal(<?php echo $category['dcmt_id']; ?>, '<?php echo htmlspecialchars($category['dcmt_name'], ENT_QUOTES); ?>', 'inventory')">
        ...
        <div class="modal-content">
            <div class="modal-header bg-danger text-white">
                <h5 class="modal-title" id="deleteModalLabel">
                    ...
                    <?php echo trans('inventory_category', 'delete_category'); ?>
                </h5>
            ...
            <div class="modal-body">
                <div class="alert alert-warning">
                    <h6 class="alert-heading">
                        <i class="fas fa-exclamation-triangle me-2"></i>
                        <?php echo trans('inventory_category', 'delete_warning'); ?>
                    </h6>
                ...
                <div class="card">
                    <div class="card-body">
                        <h6 class="card-title">
                            <i class="fas fa-info-circle me-2"></i>
                            <?php echo trans('inventory_category', 'category_information'); ?>
                        </h6>
                        <p class="mb-0">
                            <strong><?php echo trans('inventory_category', 'category_name'); ?>:</strong> 
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-danger" id="confirmDeleteBtn">
                    <i class="fas fa-trash me-2"></i><?php echo trans('inventory_category', 'delete_category'); ?>
                </button>
            </div>
        </div>
    </div>
...
window.translations = {
    ...
    confirm_delete_single: <?php echo json_encode(trans('inventory_category', 'confirm_delete_single'), JSON_UNESCAPED_UNICODE); ?>,
    confirm_delete_multiple: <?php echo json_encode(trans('inventory_category', 'confirm_delete_multiple'), JSON_UNESCAPED_UNICODE); ?>
};

window.csrfToken = <?php echo json_encode($csrf_token); ?>;

function getInventoryCategoryCsrfToken() {
    const csrfMeta = document.querySelector('meta[name="csrf-token"]');
    return (csrfMeta && csrfMeta.getAttribute('content')) || window.csrfToken || '';
}

function getInventoryCategoryCheckboxes() {
    return document.querySelectorAll('.dcmt-inventory-category-checkbox');
}

function updateBulkActions() {
    const checkboxes = getInventoryCategoryCheckboxes();
    const checkedBoxes = document.querySelectorAll('.dcmt-inventory-category-checkbox:checked');
...
function toggleSelectAll() {
    const selectAllCheckbox = document.getElementById('selectAllCheckbox');
    getInventoryCategoryCheckboxes().forEach(checkbox => {
...
function bulkDelete() {
    ...
    if (checkedBoxes.length === 0) {
        alert(<?php echo json_encode(trans('inventory_category', 'please_select_one_record'), JSON_UNESCAPED_UNICODE); ?>);
        return;
    }
...
function bulkDeleteAjax(categoryIds) {
    if (typeof showLoadingMessage === 'function') {
        showLoadingMessage(<?php echo json_encode(trans('inventory_category', 'deleting_records'), JSON_UNESCAPED_UNICODE); ?>);
    }
    ...
    fetch('bulk_delete_ajax.php', {
        ...
        body: JSON.stringify({
            ids: categoryIds,
            csrf_token: getInventoryCategoryCsrfToken()
        })
    })
    ...
    .then(data => {
        ...
        const errorMessage = data.message || <?php echo json_encode(trans('inventory_category', 'failed_to_delete_records'), JSON_UNESCAPED_UNICODE); ?>;
    ...
    .catch(error => {
        console.error('Error deleting inventory categories:', error);
        if (typeof hideLoadingMessage === 'function') {
            hideLoadingMessage();
        }
        const errorMessage = <?php echo json_encode(trans('inventory_category', 'error_occurred_deleting_records'), JSON_UNESCAPED_UNICODE); ?>;
        if (typeof showErrorMessage === 'function') {
            showErrorMessage(errorMessage);
        } else {
            showAlert('danger', errorMessage);
...
function deleteCategory(categoryId, categoryType) {
    ...
    const endpoint = categoryType === 'expense' ? '../expense_categories/delete_ajax.php' : 'delete_ajax.php';
